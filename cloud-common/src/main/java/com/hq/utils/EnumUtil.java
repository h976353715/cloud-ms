...
public class EnumUtil {
    ...
    /**
     * 获取枚举的值 返回map
     * 该方法需要在枚举类中实现 String getCode() ,String getMsg() 方法
     *
     * @param typeName 枚举类的前缀 如:获取SubjectEnum枚举值需传入Subject
     * @return {"code":"value"}
     */
    public static Map covertToMap(String typeName) {
        try {
            ...
            Method getMsg = onwClass.getMethod("getMsg");
            Method name = onwClass.getMethod("name");

            //得到enum的所有实例
            Object[] objs = onwClass.getEnumConstants();
            for (Object obj : objs) {
                map.put(String.valueOf(getCode.invoke(obj)), String.valueOf(getMsg.invoke(obj)));
            }
            return map;
        } catch (ClassNotFoundException | NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
            logger.error(e.getMessage());
